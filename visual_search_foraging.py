...
horizontal_start = -30 # horiz dimension of the grid
horizontal_end = 30 # 

vertical_start = -12 #vertical limits -20
vertical_end = 26 # 20

jitter_offset_list = np.linspace(-0.15, 0.15, 10)#[-0.2, 0, 0.2] # offset of jitter 
...
def show_intro(win):
    ...
    Gib Bescheid, wenn du bereit bist zu starten.''')
...
def run_trial(win,
    ...
    clock= visual.TextStim(win, pos=[-390, -270],units='pix',height=20,color='white')#430
    

    
    collection_count=0
    gain= visual.TextStim(win, pos=[390, -270], units='pix',height=20,color='white') #430
...
def run_experiment(n_training,max_dur):
    ...
    #else:
    win = visual.Window([width,height],monitor="testMonitor", units="pix", color=bg_color,fullscr=True,screen=1)
